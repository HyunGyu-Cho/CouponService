...
V3 비교 실험(2026-09-15)에서 같은 SQL로 V2와 V3의 사후 상태를 모두 기록했다. [V3 부하 테스트 결과](../v3/load-test-result.md) 참고.
...
V2 실험 시점의 k6 스크립트는 다음 한계가 있었다. 아래 분리 지표와 환경변수 전달은 V3 실험 전에 스크립트에 반영했다 (실행 방법은 README "부하 테스트 실행").
