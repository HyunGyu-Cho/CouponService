...
상태: 완료 (문제 재현, 구현, 부하 실험, 판단까지 마쳤다). 수치는 [V4 부하 테스트 결과](load-test-result.md) 1부와 2부에 있다.
...
가설의 세 주장을 같은 날 측정한 V3와 비교해 채점한 결과다. 수치의 정본은 [V4 부하 테스트 결과](load-test-result.md) 2부이며 여기에 옮겨 적지 않는다.

| 가설의 주장 | 결과 | 근거 |
|---|---|---|
| `coupon` 행 경합이 사라진다 | **확인** | V4 다섯 번의 실행 모두 `Innodb_row_lock_waits` 증가분 0. 같은 날 V3는 도착률 400에서 요청의 21.7%, 800에서 99.97%가 행 잠금을 기다렸다 |
| 호송 현상이 약해진다 | **확인** | 400에서 V3는 정지 한 번에 `pending` 이 189까지 치솟았고, V4는 30초 내내 0이었다. p95 36.76ms → 13ms, 최대 850ms → 106ms |
| 포화 지점이 올라간다 | **반증** | 800에서 V3 580.6/s, V4 583~651/s. 둘 다 같은 자리에서 막혔다 |

정합성은 포화 상태에서도 유지됐다. 800에서 요청 수천 건이 1초 넘게 밀리고 5,765건이 드롭된 실행에서도 `total_quantity - (GET stock) = count(coupon_issue)` 와 `SCARD issued = count(coupon_issue)` 가 모두 맞았다.

1부의 V3 수치는 비교에 쓰지 않았다. 같은 V3를 오늘 다시 돌렸더니 800에서 799.9/s·3.59ms 가 580.6/s·1.41s 로 나왔기 때문이다. 이틀 사이 실행 환경이 달라졌고, 근거와 의심되는 원인은 2부 "1부와 비교할 수 없는 이유" 에 있다.

## 판단

**복잡도를 감수할 가치가 있었다. 단, "`coupon` 행 경합이 실제 병목인 규모에서" 라는 조건이 붙는다.**

### 증명한 것과 증명하지 못한 것

이 실험은 "V4가 더 빠르다" 를 증명하지 못했다. 증명한 것은 "V4가 행 경합을 없앤다" 와 "꼬리 지연이 짧아진다" 둘이다. 처리율은 늘지 않았다.

처리율이 늘지 않은 이유는 행 잠금이 병목이 아니어서가 아니다. 같은 날 V3의 800 실행에서 요청의 99.97%가 행 잠금을 기다린 것은 사실이다. 그 앞에 더 낮은 천장이 하나 더 있었고, 행 잠금을 치우자 그 천장이 드러났을 뿐이다. 그것이 무엇인지는 이 단계에서 좁히지 못했다. 커넥션 풀이 아니라는 것과 Redis 왕복이 아니라는 것까지만 확인했다.

### 얻은 것

꼬리 지연이 실제로 나아졌다. 도착률 400에서 p95는 2.8배, 최대는 8배 짧아졌다. 선착순 쿠폰에서 사용자가 체감하는 것은 평균이 아니라 "나는 왜 혼자 오래 기다렸나" 쪽이므로, 이 개선은 처리율 수치보다 의미가 있다.

그리고 재고 판정을 DB 밖으로 옮긴 것은 로드맵의 전제다. V5 Kafka 비동기 발급은 "DB에 쓰기 전에 발급 여부를 정할 수 있다" 를 가정하며, V4가 그 자리를 만든다.

### 치른 비용

- Lua 스크립트, 지연 초기화 절차, 원인별 보상 분기가 새로 생겼다.
- Redis 운영 전제가 생겼다. 재고 키가 사라지면 안 되므로 `noeviction` 이 필요하고, 키 수명은 아직 정하지 않았다.
- 테스트와 CI가 Redis에 의존한다.
- **두 저장소가 어긋날 수 있는 구멍이 열렸다.** Redis는 성공했는데 DB 저장과 보상이 모두 실패하면 되돌릴 방법이 없다. 지금은 "최선의 노력" 보상에 기대고 있으며, 이 빚은 로드맵의 "실험: Redis 성공 후 DB 실패" 와 V8 Reconciliation 까지 열려 있다.
- `GET /api/coupons/{couponId}` 의 `remainingQuantity` 가 실시간 값이 아니게 됐다.

### 되돌릴 수 있다

`coupon.issue.version=v3` 한 줄로 V3로 돌아간다. 구현을 버전으로 나눠 둔 덕분에 이 판단은 되돌릴 수 있는 판단이고, 그만큼 가볍게 내릴 수 있다.

### 실제 서비스에서도 유지할 것인가

유지한다. 다만 조건부다. 단일 쿠폰에 요청이 몰려 `coupon` 행이 실제로 병목인 규모라면 값어치가 있다. 그 정도 부하가 아니라면 V3의 조건부 UPDATE 가 더 단순하고 충분하며, 두 저장소 정합성이라는 빚도 지지 않는다.

### 이 실험의 한계

애플리케이션, MariaDB, Redis 컨테이너, k6 가 한 대에서 돈다. 더 근본적으로 **V4를 측정하려면 Docker 가 켜져 있어야 하는데, Docker 를 켜는 것 자체가 환경을 바꾼다.** 이 한 대에서 V3와 V4를 완전히 동등한 조건으로 비교할 수 없다는 뜻이며, 2부의 비교를 같은 날 같은 세션 안에서 수행한 이유가 이것이다. 절대 수치가 아니라 같은 세션 안의 대비만 근거로 삼았다.
